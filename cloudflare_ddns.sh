...
init_config() {
    ...
    while true; do
        ...
        read -p "  对应的代理状态(小黄云) (多个用逗号分隔,如 false,true) (默认全为 false): " current_proxieds
    ...
    echo "" >> "$CONFIG_FILE"
    echo "$(declare -p ZONE_IDS | sed 's/declare -a/declare -g -a/')" >> "$CONFIG_FILE"
    echo "$(declare -p ZONE_REMARKS | sed 's/declare -a/declare -g -a/')" >> "$CONFIG_FILE"
    echo "$(declare -p RECORD_NAMES | sed 's/declare -a/declare -g -a/')" >> "$CONFIG_FILE"
    echo "$(declare -p RECORD_TYPES_SETTINGS | sed 's/declare -a/declare -g -a/')" >> "$CONFIG_FILE"
    echo "$(declare -p PROXIED_SETTINGS | sed 's/declare -a/declare -g -a/')" >> "$CONFIG_FILE"
